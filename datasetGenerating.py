            ...
            if peopleCounter != 1:            
                ...
                if xVFOA + yVFOA == -2: #If the targeted object was not found, x and y VFOA remained with its original values (-1 and -1)
                    ...
                    if (resultingVector[8:12].count(-1) > 1) or resultingVector.count(-1) >= 6:  #this tolerance value can be changed          
                        ...
                        f= open(os.path.join(os.path.abspath(os.path.join(os.getcwd(), os.pardir)), 'dataset' +  str(date.today()) , str(date.today()) + '_' + videoname+'_'+sys.argv[2]+'_'+str(j)+'.txt'),"w+") 
                        f.writelines(str(resultingVector))
                        f.close()   
                        if visualFeedback: cv2.imwrite(os.path.join(os.path.abspath(os.path.join(os.getcwd(), os.pardir)), 'dataset' +  str(date.today()), str(date.today()) + '_' + videoname+'_'+sys.argv[2]+'_'+str(j)+'.png'), frame)
                        usedFrames += 1
